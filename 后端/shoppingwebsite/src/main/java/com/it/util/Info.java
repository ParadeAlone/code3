...
public class Info {
    ...
    public static double[] createMountainArray(double[] original) {
        int length = original.length;
        int mid = length / 2;
        double[] mountain = new double[length];

        // 对于奇数长度，不需要特殊处理
        if (length % 2 == 1) {
            System.arraycopy(original, 0, mountain, 0, length);
            return mountain;
        }

        // 对于偶数长度，将中间两个元素对称放置
        mountain[mid] = original[length / 2 - 1];
        mountain[mid + 1] = original[length / 2 - 1];
        ...
        while (left < mid && right > mid) {
            mountain[left] = original[right];
            mountain[right] = original[left];
            left++;
            right--;
            index += 2;
        }
